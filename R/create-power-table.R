...
#' @export
create_power_table <- function(power = seq(0.6, 0.95, 0.05),
                               n_vec = seq(20, 100, 10),
                               bonferroni = 1000) {
  ...
  ret$power <- sapply(n_vec, function(s)
                      sapply(delta_vec, function(x)
                             power.t.test(n = s, delta = x, power = NULL,
                                          sig.level = 0.05 / bonferroni)$power)) |>
               data.frame() |> setNames(sprintf("n=%i", n_vec))
...
#' @export
plot.power_table <- function(x, plot_power = TRUE, ...) {

  withr::local_par(list(par_def))
